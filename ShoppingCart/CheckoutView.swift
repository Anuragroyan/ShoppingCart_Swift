...
struct CheckoutView: View {
    ...
    // MARK: ✅ Confirm Order Button with Firestore Save
    private var confirmButton: some View {
        Button(action: {
            cartModel.saveOrderToFirebase(
                paymentMethod: selectedPaymentMethod,
                cardType: cardType,
                maskedCardNumber: cardNumber.isEmpty ? nil : String(cardNumber.suffix(4)).padLeft(with: "*", toLength: 16)
            )

            showConfirmation = true
            cartModel.clearCart()
            cartModel.clearFavorites()
            cardNumber = ""
            expiryDate = ""
            cvv = ""
...
// MARK: - Padding Extension for Masking
extension String {
